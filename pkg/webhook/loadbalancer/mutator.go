...
//  1. Explicit Network: Uses the value from AnnotationKeyGuestClusterNetworkNameOnLB if present.
//  2. Management Network: Uses the value from AnnotationKeyGuestClusterManagementNetworkOnLB if present.
//  3. Discovery (Modern): Matches VMIs using both creator and cluster-name labels to ensure
...
func (m *mutator) findNetwork(lb *lbv1.LoadBalancer) (string, error) {
	...
	if clusterName == "" {
		...
		//
		// This "No-Break" approach provides superior troubleshooting info, as the
		// resulting IPAM error e.g. ("can't find a pool for network") is easily
		// visible to the user via 'kubectl describe'.
	...
	modernSelector := utils.NewGuestClusterNameAndCreatorNameSelector(clusterName)
	...
	legacySelector := utils.NewGuestClusterCreatorSelector()
